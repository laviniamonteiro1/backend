...
async def register_and_login_test_user(client, email: str, password: str, name: str = "Test User", role: str = "user"):
    register_response = await client.post(
        "/users/register",
        json={"name": name, "email": email, "password": password, "role": role},
    )
    if register_response.status_code == 400 and "User with this email already exists" in register_response.json().get("detail", ""):
        pass
    else:
...
async def test_create_and_get_reservation(client):
    user_token, _ = await register_and_login_test_user(client, "res_test_user@example.com", "ResTest@123!")
...
async def test_create_reservation_invalid_data_format(client):
    user_token, _ = await register_and_login_test_user(client, "create_inv_user@example.com", "InvUser@123!")
    ...
    assert any("String should match pattern" in error.get("msg", "") for error in create_response.json()["detail"])


@pytest.mark.asyncio
async def test_create_reservation_internal_server_error(client, mocker):
    user_token, _ = await register_and_login_test_user(client, "err_user_create@example.com", "ErrUser@123!")
...
async def test_get_reservation_not_found(client):
    user_token, _ = await register_and_login_test_user(client, "get_not_found_user@example.com", "NotFound@123!")
...
async def test_get_reservation_unauthorized_user(client):
    user1_token, user1_id = await register_and_login_test_user(client, "user1_get@example.com", "User1Get@123!")
    ...
    create_response1 = await client.post(
        ...
        json={
            "title": "Reserva User 1", "address": "End User 1",
    ...
    user2_token, _ = await register_and_login_test_user(client, "user2_get@example.com", "User2Get@123!")
    get_response_unauth = await client.get(
        f"/reservations/{reservation_id1}",
        headers={"Authorization": f"Bearer {user2_token}"},
    )
    assert get_response_unauth.status_code == 403
    assert get_response_unauth.json()["detail"] == "Not authorized to view this reservation"


@pytest.mark.asyncio
async def test_update_reservation_not_found(client):
    user_token, _ = await register_and_login_test_user(client, "update_not_found_user@example.com", "UpdateNF@123!")
...
async def test_delete_reservation_not_found(client):
    user_token, _ = await register_and_login_test_user(client, "delete_not_found_user@example.com", "DeleteNF@123!")
...
async def test_get_reservations_by_user_id_no_reservations(client):
    user_token, _ = await register_and_login_test_user(client, "no_res_user@example.com", "NoRes@123!")
...
async def test_get_reservations_by_user_id_with_reservations(client):
    user_token, _ = await register_and_login_test_user(client, "has_res_user@example.com", "HasRes@123!")
    ...
    for i in range(2):
        ...
        create_response = await client.post(
            ...
            headers={"Authorization": f"Bearer {user_token}"},
            json={
                "title": f"Reserva {i+1}", "address": f"Rua {i+1}",
                "check_in": check_in_dt.strftime("%d/%m/%Y às %Hh%M"),
                "check_out": check_out_dt.strftime("%d/%m/%Y às %Hh%M"),
            },
        )
    ...
    assert list_response.status_code == 404
    assert list_response.json()["detail"] == "Reservation not found" # Assume que a mensagem será essa para o 404


@pytest.mark.asyncio
async def test_update_reservation(client):
    user_token, _ = await register_and_login_test_user(client, "update_user@example.com", "Update@123!")
...
async def test_update_reservation_unauthorized_user(client):
    user1_token, _ = await register_and_login_test_user(client, "user1_upd@example.com", "User1Upd@123!")
    check_in_dt1 = datetime.datetime.now() + datetime.timedelta(days=10)
    check_out_dt1 = datetime.datetime.now() + datetime.timedelta(days=15)
    create_response1 = await client.post(
        "/reservations/",
        headers={"Authorization": f"Bearer {user1_token}"},
        json={
            "title": "Reserva User 1 Update", "address": "End User 1 Update",
            "check_in": check_in_dt1.strftime("%d/%m/%Y às %Hh%M"),
            "check_out": check_out_dt1.strftime("%d/%m/%Y às %Hh%M"),
        },
    )
    ...
    user2_token, _ = await register_and_login_test_user(client, "user2_upd@example.com", "User2Upd@123!")
    ...
    assert update_response_unauth.json()["detail"] == "Not authorized to update this reservation"


@pytest.mark.asyncio
async def test_update_reservation_fail_to_update(client, mocker):
    user_token, _ = await register_and_login_test_user(client, "fail_upd_user@example.com", "FailUpd@123!")
    ...
    create_response = await client.post(
        ...
        headers={"Authorization": f"Bearer {user_token}"},
        json={
            "title": "Reserva para falhar atualização", "address": "Rua fail",
            "check_in": check_in_dt.strftime("%d/%m/%Y às %Hh%M"),
            "check_out": check_out_dt.strftime("%d/%m/%Y às %Hh%M"),
        },
    )
    ...
    mocker.patch('blog.usecases.reservation.update_reservation.UpdateReservation.execute', return_value=None)
    ...
    assert "Reservation not found or could not be updated" in update_response.json()["detail"]


@pytest.mark.asyncio
async def test_update_reservation_internal_server_error(client, mocker):
    user_token, _ = await register_and_login_test_user(client, "err_upd_user@example.com", "ErrUpd@123!")
    ...
    create_response = await client.post(
        ...
        headers={"Authorization": f"Bearer {user_token}"},
        json={
            "title": "Reserva para erro interno", "address": "Rua erro internal",
            "check_in": check_in_dt.strftime("%d/%m/%Y às %Hh%M"),
            "check_out": check_out_dt.strftime("%d/%m/%Y às %Hh%M"),
        },
    )
    ...
    mocker.patch('blog.usecases.reservation.update_reservation.UpdateReservation.execute', side_effect=Exception("Database error during update"))
...
async def test_cancel_reservation(client):
    user_token, _ = await register_and_login_test_user(client, "cancel_user@example.com", "Cancel@123!")
...
async def test_cancel_reservation_unauthorized_user(client):
    user1_token, _ = await register_and_login_test_user(client, "user1_cancel@example.com", "User1Cancel@123!")
    check_in_dt1 = datetime.datetime.now() + datetime.timedelta(days=10)
    check_out_dt1 = datetime.datetime.now() + datetime.timedelta(days=15)
    create_response1 = await client.post(
        "/reservations/",
        headers={"Authorization": f"Bearer {user1_token}"},
        json={
            "title": "Reserva User 1 Cancel", "address": "End User 1 Cancel",
            "check_in": check_in_dt1.strftime("%d/%m/%Y às %Hh%M"),
            "check_out": check_out_dt1.strftime("%d/%m/%Y às %Hh%M"),
        },
    )
    ...
    user2_token, _ = await register_and_login_test_user(client, "user2_cancel@example.com", "User2Cancel@123!")
    ...
    assert cancel_response_unauth.json()["detail"] == "Not authorized to cancel this reservation"


@pytest.mark.asyncio
async def test_cancel_reservation_fail_to_cancel(client, mocker):
    user_token, _ = await register_and_login_test_user(client, "fail_cancel_user@example.com", "FailCancel@123!")
    ...
    create_response = await client.post(
        ...
        headers={"Authorization": f"Bearer {user_token}"},
        json={
            "title": "Reserva para falhar cancelamento", "address": "Rua cancel fail",
            "check_in": check_in_dt.strftime("%d/%m/%Y às %Hh%M"),
            "check_out": check_out_dt.strftime("%d/%m/%Y às %Hh%M"),
        },
    )
    ...
    mocker.patch('blog.usecases.reservation.cancel_reservation.CancelReservation.execute', return_value=None)
    ...
    assert "Reservation not found or could not be cancelled" in cancel_response.json()["detail"]


@pytest.mark.asyncio
async def test_cancel_reservation_internal_server_error(client, mocker):
    user_token, _ = await register_and_login_test_user(client, "err_cancel_user@example.com", "ErrCancel@123!")
    ...
    create_response = await client.post(
        ...
        headers={"Authorization": f"Bearer {user_token}"},
        json={
            "title": "Reserva para erro interno cancel", "address": "Rua erro internal cancel",
            "check_in": check_in_dt.strftime("%d/%m/%Y às %Hh%M"),
            "check_out": check_out_dt.strftime("%d/%m/%Y às %Hh%M"),
        },
    )
    ...
    mocker.patch('blog.usecases.reservation.cancel_reservation.CancelReservation.execute', side_effect=Exception("Database error during cancel"))
...
async def test_list_all_reservations_admin_access(client):
    admin_token, admin_user_id = await register_and_login_test_user(client, "admin_res@example.com", "AdminRes@123!", role="admin")
...
async def test_list_all_reservations_user_forbidden(client):
    user_token, _ = await register_and_login_test_user(client, "forbidden_user@example.com", "Forbidden@123!")
    ...
    assert forbidden_response.json()["detail"] == "Only admins can list all reservations"


@pytest.mark.asyncio
async def test_delete_reservation(client):
    user_token, _ = await register_and_login_test_user(client, "delete_user@example.com", "Delete@123!")
...
async def test_delete_reservation_unauthorized_user(client):
    user1_token, _ = await register_and_login_test_user(client, "user1_del@example.com", "User1Del@123!")
    check_in_dt1 = datetime.datetime.now() + datetime.timedelta(days=10)
    check_out_dt1 = datetime.datetime.now() + datetime.timedelta(days=15)
    create_response1 = await client.post(
        "/reservations/",
        headers={"Authorization": f"Bearer {user1_token}"},
        json={
            "title": "Reserva User 1 Delete", "address": "End User 1 Delete",
            "check_in": check_in_dt1.strftime("%d/%m/%Y às %Hh%M"),
            "check_out": check_out_dt1.strftime("%d/%m/%Y às %Hh%M"),
        },
    )
    ...
    user2_token, _ = await register_and_login_test_user(client, "user2_del@example.com", "User2Del@123!")
    delete_response_unauth = await client.delete(
        f"/reservations/{reservation_id1}",
        headers={"Authorization": f"Bearer {user2_token}"},
    )
    assert delete_response_unauth.status_code == 403
    assert delete_response_unauth.json()["detail"] == "Not authorized to delete this reservation"


@pytest.mark.asyncio
async def test_delete_reservation_fail_to_delete(client, mocker):
    user_token, _ = await register_and_login_test_user(client, "fail_del_user@example.com", "FailDel@123!")
    ...
    create_response = await client.post(
        ...
        headers={"Authorization": f"Bearer {user_token}"},
        json={
            "title": "Reserva para falhar exclusão", "address": "Rua del fail",
            "check_in": check_in_dt.strftime("%d/%m/%Y às %Hh%M"),
            "check_out": check_out_dt.strftime("%d/%m/%Y às %Hh%M"),
        },
    )
    ...
    mocker.patch('blog.usecases.reservation.delete_reservation.DeleteReservation.execute', return_value=False)
    ...
    assert delete_response.status_code == 404
    assert delete_response.json()["detail"] == "Reservation not found or could not be deleted"